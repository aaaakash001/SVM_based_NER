...
all_words = [token for sentence in dataset['train'] for token in sentence]
vocab_count = Counter(all_words)
# Define a threshold for considering words as unknown
vocab_threshold = 3  # Consider words with a frequency < 5 as unknown
known_words = set(word for word, count in vocab_count.items() if count >= vocab_threshold)

# Modify the word_features function to add an "is_unknown" feature
def word_features(sentence, i, chunk, pos_tags, ner_tags):
    ...
    features = {
        ...
        # New feature to handle unknowns
        'is_unknown': int(word not in known_words)
    }
    return features
...
for fold, (train_index, test_index) in enumerate(tqdm(kf.split(X), total=kf.n_splits, desc="Training Progress")):
    ...
    model = svm.SVC(C=2, kernel='linear', gamma='scale', cache_size=2000)
    ...
    if fold + 1 == 1:
        with open('./param/best_ner_model_unk.pkl', 'wb') as f:
            pickle.dump(model, f)
        with open('./param/dict_vectorizer_unk.pkl', 'wb') as vec_file:
